...
import { Plus, Edit, Trash2, Eye, MapPin, Clock, DollarSign, Star, Calendar, Filter, Search, Users, Settings } from 'lucide-react';
import { SAMPLE_SERVICES } from '@/data/hostsData';

interface ServiceManagementProps {
  hostType: string;
}

export default function ServiceManagement({ hostType }: ServiceManagementProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button className="flex items-center space-x-2 px-4 py-2 border border-serai-navy-300 rounded-lg hover:bg-serai-navy-50 text-serai-navy-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredServices.map((service) => (
          <div key={service.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
            {/* Service Image */}
            <div className="h-48 bg-gray-200 relative">
              ...
              <div className="absolute top-2 left-2">
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(service.type)}`}>
                  {service.type}
                </span>
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-gray-600 ml-1">/service</span>
                </div>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            {searchTerm || filterStatus !== 'all' || filterType !== 'all'
              ...
              : 'Get started by offering your first service.'
            }
          </p>
          {!searchTerm && filterStatus === 'all' && filterType === 'all' && (
            <button
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Offer New Service</h3>
            <p className="text-gray-600 mb-6">This modal would contain the service creation form.</p>
